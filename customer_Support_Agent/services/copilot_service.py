...
from customer_Support_Agent.integration.memory.mem0_store import (
    CustomeMemoryStore
)
from customer_Support_Agent.integration.rag.chroma_kb import knowledgeBaseService
from customer_Support_Agent.integration.tools.support_tools import get_support_tools


class SupportCopilot:
    def __init__(self, settings: Settings) -> None:
        if not settings.OPENAI_API_KEY:
            raise RuntimeError("OPENAI_API_KEY is missing add it in .env before geberating the graph")
        ...
        try:
            self.memory = CustomeMemoryStore(self._settings, self._llm)
        except Exception as exc:
            self._memory_error = str(exc)
            logging.error(f"Failed to create memory store: {exc}")
            
        self.rag = knowledgeBaseService(self._settings)
