...
from customer_Support_Agent.repositories.sqlite.drafts import DraftRepository
...
def list_tickets(limit: int = 100) -> list[dict[str, Any]]:
    return _ticket.list(limit=limit)

def get_ticket_by_id(ticket_id: int) -> dict[str, Any] | None:
    return _ticket.get_by_id(ticket_id)

def set_ticket_status(ticket_id: int, status: str) -> dict[str, Any] | None:
    return _ticket.set_status(ticket_id=ticket_id, status=status)

def count_open_tickets_for_customer(customer_email: str) -> int:
    return _ticket.count_open_for_customer(customer_email)
...
def create_draft(
    ...
    return _draft.create(ticket_id=ticket_id, content=content, context_used=context_used, status=status)

def get_latest_draft_for_ticket(ticket_id: int) -> dict[str, Any] | None:
    return _draft.get_latest_for_ticket(ticket_id=ticket_id)

def get_draft_by_id(draft_id: int) -> dict[str, Any] | None:
    return _draft.get_by_id(draft_id=draft_id)

def update_draft(draft_id: int,
                content: str | None = None,
                status: str | None = None
        ) -> dict[str, Any] | None:
    return _draft.update(draft_id=draft_id, content=content, status=status)

def get_ticket_and_customer_by_draft(draft_id: int) -> dict[str, Any] | None:
    return _draft.get_ticket_and_customer_by_draft(draft_id=draft_id)
