...
from customer_Support_Agent.schemas.api import TicketCreateRequest, TicketResponse, GenerateDraftRequest
...
def _generate_and_store_draft_background(
    ...
    return draft_service.generate_store_background(
        ticket_id=ticket_id,
...
@router.get("/api/tickets/{ticket_id}/generate-draft", response_model=TicketResponse)
def generate_draft_route(
    ...
    try:
        draft = draft_service.gnerate_and_store_manual(
            ticket_id=ticket_id,
            ticket = ticket,
            customer=customer,
            draft_repository=draft_repository,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate draft for {exc}") from exc
    
    return {
        "ticket_id": ticket_id,
        "draft": draft_service.serialize_ticket(ticket)
    }


@router.post("/api/tickets/{ticket_id}/draft", response_model=TicketResponse)